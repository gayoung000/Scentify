 ...
 * @returns 성공 시 초대 코드 및 초대 링크를 반환, 실패 시 null 반환
 */
export const createInviteCode = async (
  deviceId: string,
  accessToken: string
): Promise<CreateInviteCodeResponse | null> => {
  try {
    // 초대 코드 생성 API 호출
    const response = await fetch("/v1/group/invite/create", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ deviceId }), // 요청에 디바이스 ID 포함
    });
